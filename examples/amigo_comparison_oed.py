...
init_biomass = cs.SX.sym('init_biomass')
init_substrate = cs.SX.sym('init_substrate')
...
inputs = cs.vertcat(initial_states)
...
input_names = ["Init_Biomass","Init_Substrate"]
...
fit_params = [mumax,ks,kd,yld]

init_inputs = pd.DataFrame({'Init_Biomass':[0.5]*20,
                            'Init_Substrate':[1.5]*20,
                            'Variable':['biomass_t'+"{0:0=2d}".format(i)+"_00" for i in range(1,11)]
                                        +['substrate_t'+"{0:0=2d}".format(i)+"_00" for i in range(1,11)]})
...
time_sim = [float(i) for i in range (1,11)]
...
plt.plot(time_sim,predictions[('Prediction','Mean')].iloc[0:10])
#plot biomass data
plt.scatter(time_dat,biomass_dat)
#plot biomass sim dat
plt.plot(time_sim,samp_dat[('Observation')].iloc[0:10],'.')

#plot substrate simulation
plt.plot(time_sim,predictions[('Prediction','Mean')].iloc[10:20])
#plot substrate data
plt.scatter(time_dat,substrate_dat)
#plot substrate sim dat
plt.plot(time_sim,samp_dat[('Observation')].iloc[10:20],'x')
plt.show()

init_data = init_inputs.copy()
init_data["Observation"] = biomass_dat[1:11] + substrate_dat[1:11]

## Supplemental Listing 18 ##############################################

#set up the design algorithm to use continuous (continuous) optimization with two unique inputs points
continuous_inputs={'Inputs':['Init_Biomass','Init_Substrate'],
                   'Bounds':[(1,5),(5,40)],
                   'Structure':[['cb0','cs0']]}
#set up fixed design dictionary
fixed_dict ={'Weight':0.5,'Design':init_design}
#fix observation groups so that biomass and substrate at each timepoint are observed together
observ_groups = [["biomass_t"+"{0:0=2d}".format(i)+"_00","substrate_t"+"{0:0=2d}".format(i)+"_00"] for i in range(1,11)]
# generate the optimal discreteimate (relaxed) design
design_object = nl.Design(model_object,fit_params,'D',
